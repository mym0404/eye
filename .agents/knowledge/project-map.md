...
- `src/query/definitions.ts` and `src/query/references.ts`: semantic-first plus index/fallback resolution strategy.
...
- `tests/ts-navigation.test.ts` and `tests/python-navigation.test.ts`: language-specific semantic-first navigation.
- `tests/mcp-server.e2e.ts`: stdio MCP surface, lazy runtime creation, scoped queries, and persisted cache behavior.
- `tests/mcp-server.real-fixtures.e2e.ts`: pinned real-repository coverage for structure, source, status, and selected semantic-capable symbol flows.
