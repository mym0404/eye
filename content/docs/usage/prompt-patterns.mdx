...
- `Use eye to inspect this repository before answering. Resolve createProgram with query_symbol target: "createProgram", then follow references using symbolId and summarize the main call sites.`
- `Use eye only inside packages/next/src/server. Refresh the index for that scope, then use query_symbol target: "loadConfig" for definition and references.`
